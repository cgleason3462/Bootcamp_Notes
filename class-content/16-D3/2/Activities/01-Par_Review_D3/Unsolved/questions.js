...
// Creates a virtual selection of a list
var selection = ul.selectAll("li")
    // bind the data
    .data(arr)
    // start work from the end of the current rows
    // Since we have no rows we use .enter()
    .enter()
    // append a li element for each of the items in the array
    .append("li")
    // Set the text of each element to match the items in the array
    .text(function(d){
        return d
});
